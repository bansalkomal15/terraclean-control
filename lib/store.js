...
const redisDriver = {
  ...
  async casDoc(expectedRev, doc) {
    try {
      ...
      /* Some Redis hosts refuse scripts. Fall back to read-then-write: a shade
         less airtight under simultaneous saves, but saving keeps working. */
      console.log('[store] script unavailable, using read-then-write:', e.message);
      const raw = await redis(['GET', DOC_KEY]);
      if (raw) {
        try { if (JSON.parse(raw).rev !== expectedRev) return false; } catch (err) { }
      }
      await redis(['SET', DOC_KEY, JSON.stringify(doc)]);
      return true;
    }
  },
