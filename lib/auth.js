...
function passwordProblem(pw) {
  ...
  if (!/^[a-z0-9]+$/.test(s)) return 'Use lower case letters and numbers only — no capitals, spaces or punctuation.';
  if (/^\d+$/.test(s)) return 'Use more than just numbers.';
  if (['password', '12345678', 'terraclean'].includes(s)) return 'That password is too easy to guess.';
  return null;
}
...
/* Lower case letters and digits only — nothing to mistype, nothing to explain
   over the phone, and it survives being pasted about. */
function generatePassword() {
  return PW_WORDS[crypto.randomInt(0, PW_WORDS.length)] + String(crypto.randomInt(1000, 10000));
}

/* Everything is filed under the address in lower case. Sign-in lowercases what
   is typed, so if anything else stored it as written — one capital letter in the
   directory — the password would be saved somewhere the login never looks. */
const norm = e => String(e || '').trim().toLowerCase();
const key = {
  cred: e => 'cred:' + norm(e),
  code: e => 'code:' + norm(e),
  invite: e => 'invite:' + norm(e),
  session: t => 'sess:' + t
};
